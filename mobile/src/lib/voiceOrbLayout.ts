...
export function computeVoiceOrbLayout(params: {
  ...
  const idealHaloDiameter = Math.round(baseHaloDiameter * widthScale);
  const maxContainedHaloDiameter = Math.floor((boundedWidth - 12) / haloPeakScale);
  const haloDiameter = clamp(Math.min(idealHaloDiameter, maxContainedHaloDiameter), 132, baseHaloDiameter);
  const displayedHaloDiameter = Math.round(haloDiameter * haloPeakScale);
  const coreSize = clamp(Math.round(haloDiameter * 0.6), 86, 104);
  ...
  return {
    ...
    haloPeakScale,
    haloMaxOpacity,
    stageVisualHeight: Math.max(132, displayedHaloDiameter + 18),
